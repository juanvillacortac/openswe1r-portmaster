...
GAMEDIR="/$directory/ports/openswe1r"
CONFDIR="$GAMEDIR/conf"
mkdir -p "$CONFDIR/openswe1r"
...
bind_directories "$HOME/.local/share/openswe1r" "$CONFDIR/openswe1r"

export OPENSWE1R_ROOT="$GAMEDIR/game"
[ "${DEVICE_CPU^^}" == "RK3326" ] && export OPENSWE1R_SSAA_TARGET=35

# --- CFW tweaks (ROCKNIX / Wayland) ---
[ -z "${OPENSWE1R_SWAY_FULLSCREEN+x}" ] && [ "${CFW_NAME^^}" = "ROCKNIX" ] && export OPENSWE1R_SWAY_FULLSCREEN=0
[ "${CFW_NAME^^}" = "ROCKNIX" ] && export SDL_HINT_APP_NAME="${SDL_HINT_APP_NAME:-OpenSWE1R}"
...
# Low-RAM swap (< 2 GB; SWEp1 Racer loads level + sound data into RAM)
...
$ESUDO chmod +x "$GAMEDIR/openswe1r.${DEVICE_ARCH}"

$GPTOKEYB "openswe1r.${DEVICE_ARCH}" -c "./openswe1r.gptk" &
pm_platform_helper "$GAMEDIR/openswe1r.${DEVICE_ARCH}"
./openswe1r.${DEVICE_ARCH}
